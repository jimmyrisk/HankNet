...
class RolloutStorage(object):
    def __init__(self, num_steps, input_dims_tensor, input_dims, action_space,
                 recurrent_hidden_state_size):
        self.obs = torch.zeros(num_steps + 1, *input_dims_tensor)
        self.obs_num = torch.zeros(num_steps + 1, input_dims)
    ...
    def to(self, device):
        ...
        self.obs_num = self.obs_num.to(device)
    ...
    def insert(self, obs, obs_num, recurrent_hidden_states, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks):
        self.obs[self.step + 1].copy_(obs)
        self.obs_num[self.step + 1].copy_(obs_num)
        self.recurrent_hidden_states[self.step +
                                     1].copy_(recurrent_hidden_states)
        self.actions[self.step].copy_(actions[0])
        self.action_log_probs[self.step].copy_(action_log_probs[0])
    ...
    def after_update(self):
        ...
        self.obs_num[0].copy_(self.obs_num[-1])
    ...
    def feed_forward_generator(self,
        ...
        for indices in sampler:
            ...
            yield obs_batch, obs_num_batch, recurrent_hidden_states_batch, actions_batch, \
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_envs_per_batch):
            ...
            obs_num_batch = torch.stack(obs_num_batch, 1)
            ...
            obs_num_batch = _flatten_helper(T, N, obs_num_batch)
